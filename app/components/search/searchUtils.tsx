...
const GeoAutocomplete: React.FC<SearchProps> = (props) => {
  _isMounted = false;
  _results = [];
  _requests = [];
  constructor(props) {
  ...
  componentDidMount() {
    ...
    // been rendered
    this.mapStore = this.context.applicationStore.mapStore;
    this.searchStore = this.context.applicationStore.searchStore;
    this.userStore = this.context.applicationStore.userStore;
  ...
  _requestNearby = (latitude: string, longitude: string) => {
    this._abortRequests();
  ...
  doSearch = async () => {
    ...
    const tagsQuery = {
      query: gql`
          query {
              search_tags(
                  args: {
                      search: "${this.searchStore.searchText}",
                      userid: "${userId}"
                  }
              ) {
                  tagId
    ...
    this.searchStore.setIsFetchingTags(true);
    fetchQuery(tagsQuery)
      .then((results) => {
        this.searchStore.setTags(results.data.search_tags);
      })
      .finally(() => this.searchStore.setIsFetchingTags(false));
  };

  _handleChangeText = (text) => {
    this._onChangeText(text);
    this.searchStore.setSearchText(text);
        ...
        <View
          ...
          <ScrollView
            ...
            style={[
              ...
              {height: Constants.height - 60, width: Constants.width},
            ]}
